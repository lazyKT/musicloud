...
const UserDetails = ({ uploadAvatar, updatedImg }) => {
  ...
  const onChangeListender = (event) => {
    if (event.target.value.length < 4) dispatch({ type: "setError" });
    else dispatch({ type: "removeError" });
  ...
  return (
    <>
      <div id="userDetails">
        ...
        {state.loaded ? (
          <UpdateUser
            onChange={onChangeListender}
            update={state.edit}
            value={state.updateVal}
            isInputChanged={state.readySumit}
            hasError={state.error}
            onClick={editHandler}
            saveOnClick={saveHandler}
            cancelHandler={cancelHandler}
            hideHeaderMsg={hideHeaderMsg}
            header={state.msgTxt}
            headerType={state.msgType}
            showHeader={state.display}
          />
        ) : (
          "loading..."
        )}
